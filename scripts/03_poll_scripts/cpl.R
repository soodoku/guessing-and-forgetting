...
"
# Load data
delib <- read.csv("data/cpl.csv")
			
# 0, 1, and NA
t1raw <- paste0("know", letters[1:7], "1raw")
t2raw <- paste0("know", letters[1:7], "2raw")

# Lucky
lucky <- c(.167, .333, .333, .333, .25, .143, .20)

# Reliability
t1_alpha <- round(ltm::cronbach.alpha(nona(delib[,t1raw]))$alpha,3)
t2_alpha <- round(ltm::cronbach.alpha(nona(delib[,t2raw]))$alpha,3)
write.table(t(c("TX Utilities: CPL", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="TX Utilities: CPL", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="TX Utilities: CPL", personID=c(1:nrow(delib)), indiv)
...
res  <- c("TX Utilities: CPL", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="TX Utilities: CPL", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
