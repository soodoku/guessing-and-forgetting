...
"
# Load data
delib <- read.csv("data/sm.csv")
	
t1raw <- paste0("t1q", 19:26, "raw")
t2raw <- paste0("t2q", 19:26, "raw")
...
# Lucky
lucky <- c(.2, .2, .2, .2, .2, .2, .25, .2)

# Reliability
t1_alpha <- round(ltm::cronbach.alpha(nona(delib[,t1raw]))$alpha,3)
t2_alpha <- round(ltm::cronbach.alpha(nona(delib[,t2raw]))$alpha,3)
write.table(t(c("San Mateo", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="San Mateo", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="San Mateo", personID=c(1:nrow(delib)), indiv)
...
res  <- c("San Mateo", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="San Mateo", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
