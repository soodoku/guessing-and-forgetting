...
"
# Load data
delib <- read.csv("data/mi.csv")
	
# Indices
t1raw 	<- c(paste0("t1q", 14:18, "raw"), "t1q4raw", "t1q5raw", "t1q7raw", "t1q8raw")
t2raw 	<- c(paste0("t3q", 38:42, "raw"), paste0("t3q", c(10,11,13,14), "raw"))

# Lucky
lucky <- c(.50, .50, .25, .25, .25, .571, .571, .571, .571)

# Reliability
t1_alpha <- round(ltm::cronbach.alpha(nona(delib[,t1raw]))$alpha,3)
t2_alpha <- round(ltm::cronbach.alpha(nona(delib[,t2raw]))$alpha,3)
write.table(t(c("Michigan", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="Michigan", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="Michigan", personID=c(1:nrow(delib)), indiv)
...
res  <- c("Michigan", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="Michigan", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
