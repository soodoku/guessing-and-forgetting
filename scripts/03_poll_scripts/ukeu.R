...
"
# Load data
delib <- read.csv("data/ukeu.csv")

# 0, 1, and NA
t1raw <- paste0(c("pk11", "pk21", "pk31", "pk41", "pk51"), "raw")
t2raw <- paste0(c("pk12", "pk22", "pk32", "pk42", "pk52"), "raw")

# Lucky		
lucky <- c(.50, .50, .50, .50, .50)

# Reliability
t1_alpha <- round(ltm::cronbach.alpha(nona(delib[,t1raw]))$alpha,3)
t2_alpha <- round(ltm::cronbach.alpha(nona(delib[,t2raw]))$alpha,3)
write.table(t(c("UK EU Poll", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="UK EU Poll", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="UK EU Poll", personID=c(1:nrow(delib)), indiv)
...
res  <- c("UK EU Poll", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="UK EU Poll", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
