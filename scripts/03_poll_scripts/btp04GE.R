...
By the People 2004 General Elections	
...
"
# Load data
delib <- read.csv("data/btp04GE.csv")
	
# Indices
t1raw 	<- paste0("t1know", 1:9, "raw")
t2raw 	<- paste0("t3know", 1:9, "raw")

# Lucky
lucky <- c(.33, .25, .50, .50, .50, .25, .25, .25, .25)		

# Reliability
t1_alpha <- round(ltm::cronbach.alpha(nona(delib[,t1raw]))$alpha,3)
t2_alpha <- round(ltm::cronbach.alpha(nona(delib[,t2raw]))$alpha,3)
write.table(t(c("By the People 2004 General Elections", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="By the People 2004 General Elections", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="By the People 2004 General Elections", personID=c(1:nrow(delib)), indiv)
...
res  <- c("By the People 2004 General Elections", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="By the People 2004 General Elections", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
