...
# Load data
delib <- read.csv("data/ukhealth.csv")

# DK Dataset: Creating dataset for 1, 0, NA only
t1raw <- c("sopha1r", "sophb1r", "sophc1r", "sophd1r", "sophe1r", "sophf1r")
t2raw <- c("sopha2r", "sophb2r", "sophc2r", "sophd2r", "sophe2r", "sophf2r")

# Lucky
lucky <- c(.50, .50, .50, .50, .50, .50)

# Reliability
t1_alpha <- round(ltm::cronbach.alpha(nona(delib[,t1raw]))$alpha,3)
t2_alpha <- round(ltm::cronbach.alpha(nona(delib[,t2raw]))$alpha,3)
write.table(t(c("UK Health", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="UK Health DP", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="UK Health DP", personID=c(1:nrow(delib)), indiv)
...
res  <- c("UK Health DP", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="UK Health DP", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
