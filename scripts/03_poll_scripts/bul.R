...
"
# Load data
delib <- read.csv("data/bul.csv")

# Vectors of Names
# 0, 1, and NA
t1raw <- paste0("t1q12_", 1:7, "r")
t2raw <- paste0("t2q12_", 1:7, "r")
...
write.table(t(c("Bulgaria", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="Bulgaria", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="Bulgaria", personID=c(1:nrow(delib)), indiv)
...
res  <- c("Bulgaria", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="Bulgaria", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
