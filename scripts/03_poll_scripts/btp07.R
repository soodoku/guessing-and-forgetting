...
# Load data
delib <- read.csv("data/btp07.csv")
	
# Indices
t1raw 	<- paste0("t1know", 1:8, "raw")
t2raw 	<- paste0("t3know", 1:8, "raw")

# Lucky	
lucky <- c(.25, .25, .25, .25, .25, .25, .25, .25)

# Reliability
t1_alpha <- round(ltm::cronbach.alpha(nona(delib[,t1raw]))$alpha,3)
t2_alpha <- round(ltm::cronbach.alpha(nona(delib[,t2raw]))$alpha,3)
write.table(t(c("By the People 2007", t1_alpha, t2_alpha)), file="results/reliability.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
...
# Write out results
# item results
res   <- cbind(poll="By the People 2007", itemID=names(stnd$learn), raw = raw, lca = lca, stnd = stnd$learn)
...
res   <- cbind(poll="By the People 2007", personID=c(1:nrow(delib)), indiv)
...
res  <- c("By the People 2007", n=nrow(delib), nitems = length(raw), raw = mean(raw), lca= mean(lca), stnd = mean(stnd$learn), 
...
write.table(t(c(poll="By the People 2007", lca=mean(res[2,] < .05))), file="results/fitresults.csv", col.names = F, row.names = F, append=T, sep=",", qmethod = "double")
